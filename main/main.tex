...
\begin{longtable}{|>{\raggedright\arraybackslash}p{5.5cm}|>{\raggedright\arraybackslash}p{7cm}|>{\raggedright\arraybackslash}p{3cm}|}
    ...
    auth:\allowbreak reset\_password:\allowbreak user\_email:\allowbreak \{email\}                                                     & Lưu token hoặc trạng thái đặt lại mật khẩu của người dùng qua email.                                              & 10 phút                          \\
    \hline
    auth:\allowbreak otp:\allowbreak user\_id:\allowbreak \{id\}:\allowbreak action:\allowbreak \{action\}                             & Lưu mã OTP và số lần thử xác thực/hành động tương ứng của người dùng.                                             & 10 phút                          \\
    \hline
    geocode:\allowbreak \{lat\},\{lng\}                                                                                                & Bộ đệm thông tin địa lý (tỉnh/thành) từ GPS (lấy 3 chữ số thập phân $\sim$110m) để giảm spam API ngoài.           & 30 ngày                          \\
    \hline
    weather:\allowbreak \{lat\},\{lng\}                                                                                                & Bộ đệm thông tin thời tiết thực địa theo GPS (lấy 2 chữ số thập phân $\sim$1.1km).                                & 30 phút                          \\
    \hline
    system\_config:\allowbreak \{key\}                                                                                                 & Cache cấu hình hệ thống động để giảm tải cho CSDL PostgreSQL.                                                     & Vô hạn                           \\
    \hline
    user:\allowbreak \{userId\}:\allowbreak route:\allowbreak \{routeIdentifier\}:\allowbreak failed-attempts                          & Đếm số lần thực hiện hành động lỗi (nhập sai mật khẩu/OTP) trên một endpoint cụ thể để phòng chống brute-force.   & 15 phút (User) / 30 phút (Admin) \\
    \hline
    rate\_limit:\allowbreak \{method\}:\allowbreak \{endpoint\}:\allowbreak \{requestIp\}                                              & Giới hạn tần suất gửi yêu cầu của IP đối với phương thức và endpoint tương ứng.                                   & 1 phút                           \\
    \hline
    semantic\_cache:\allowbreak diagnosis:\allowbreak diseases:\allowbreak \{sortedDiseases\}:\allowbreak area:\allowbreak \{areaStr\} & Cache kết quả tư vấn từ RAG (Gemini) dựa trên các bệnh hại được nhận diện và mức độ bị ảnh hưởng của vết bệnh.    & 24 giờ                           \\
    \hline
    semantic\_cache\_keys                                                                                                              & Set lưu danh sách toàn bộ các key cache ngữ nghĩa để thực hiện cơ chế xóa bộ đệm đồng loạt khi tri thức cập nhật. & Vô hạn                           \\
    \hline
\end{longtable}
